...
class VGG_student(nn.Module):
    def __init__(self,features,num_classes=10,init_weights=False):
        super(VGG_student,self).__init__()
        self.features = models.__dict__[features](pretrained=False).features
        self.classifier = nn.Sequential(  # 分类部分的网络
            nn.Linear(512,512),
            nn.Linear(512, num_classes),
...
# origin_net_data = torch.load("zhengliu_stego_0bits.pth")
# origin_net = VGG_student(features="vgg11_bn",num_classes=10,init_weights=True)
# origin_net.load_state_dict(origin_net_data)


# 加载隐写模型
# qat_model_data = torch.load("zhengliu_stego_100bits.pth")
# qat_model_data = torch.load("zhengliu_stego_200bits.pth")
# qat_model_data = torch.load("zhengliu_stego_300bits.pth")
# qat_model_data = torch.load("zhengliu_stego_400bits.pth")
# qat_model_data = torch.load("zhengliu_stego_500bits.pth")

# qat_model = VGG_student(features="vgg11_bn",num_classes=10,init_weights=True)
# qat_model.load_state_dict(qat_model_data)


# kernel_size_height = qat_model.features[4].weight.shape[2]
# kernel_size_width = qat_model.features[4].weight.shape[3]
#
# feature = origin_net.features[4].weight
# P1 = torch.reshape(feature, (-1, kernel_size_height * kernel_size_width))
#
#
# feature = qat_model.features[4].weight
...
# with open("zhengliu_x.txt","w") as f:
#     for line in x:
#         print(str(line))
#         f.write(str(line) + '\n')
#
# with open("zhengliu_origin.txt","w") as f:
#     for line in p:
#         print(str(line.item()))
#         f.write(str(line.item()) + '\n')
#
# with open("zhengliu_stego_100bit.txt","w") as f:
#     for line in q:
#         print(str(line.item()))
#         f.write(str(line.item()) + '\n')

# with open("zhengliu_stego_200bit.txt","w") as f:
#     for line in q:
#         print(str(line.item()))
#         f.write(str(line.item()) + '\n')

# with open("zhengliu_stego_300bit.txt","w") as f:
#     for line in q:
#         print(str(line.item()))
#         f.write(str(line.item()) + '\n')
...
class GcnNet(nn.Module):
    ...
    def forward(self, conv1, i, is_quantitative_model = False):
        ...
        if is_quantitative_model:
            ...
            logits = self.linear(feature)
            # 上面两句合成一句
            # logits = self.linear(feature.dequantize())
            # print(logits)
...
# gcn_model = torch.load("zhengliu_gcn_100bits.pth")
gcn_model = torch.load("zhengliu_gcn_200bits.pth")
# gcn_model = torch.load("zhengliu_gcn_300bits.pth")
# gcn_model = torch.load("zhengliu_gcn_400bits.pth")
# gcn_model = torch.load("zhengliu_gcn_500bits.pth")


# secret_tensor = torch.load("zhengliu_secret_100bits.pkl")
# secret_tensor = torch.load("zhengliu_secret_200bits.pkl")
# secret_tensor = torch.load("zhengliu_secret_300bits.pkl")
# secret_tensor = torch.load("zhengliu_secret_400bits.pkl")
# secret_tensor = torch.load("zhengliu_secret_500bits.pkl")
...
# with open("zhengliu_gaosi_100bit.txt", "w") as f:
# with open("zhengliu_junyun_100bit.txt", "w") as f:
# with open("zhengliu_gaosi_200bit.txt", "w") as f:
# with open("zhengliu_junyun_200bit.txt", "w") as f:
# with open("zhengliu_gaosi_300bit.txt", "w") as f:
# with open("zhengliu_junyun_300bit.txt", "w") as f:
# with open("zhengliu_gaosi_400bit.txt", "w") as f:
# with open("zhengliu_junyun_400bit.txt", "w") as f:
# with open("zhengliu_gaosi_500bit.txt", "w") as f:
# with open("zhengliu_junyun_500bit.txt", "w") as f:
#     for i in range(1, 31):
#         i=i*0.01
#         print(i)
#         gcn_out = gcn_model(qat_model.features[4], i, is_quantitative_model=False)
...
#for i in range(43):
for i in range(5):
    qat_model_data = torch.load("./zhengliu_stego_200bits_jit" + str(i) + ".pth")
    qat_model = VGG_student(features="vgg11_bn", num_classes=10, init_weights=True)
    qat_model.load_state_dict(qat_model_data.state_dict())

    gcn_out = gcn_model(qat_model.features[4], 0, is_quantitative_model=False)
    Y_decision = decision(gcn_out)
